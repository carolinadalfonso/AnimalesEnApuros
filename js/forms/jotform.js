...
var JotForm = {
  ...
  paymentTexts: {
    ...
    taxTax: "Tax",
    notAvailableStock:
      "Not available stock for some of the product(s), Please update your selection(s)",
  },
  validationRegexes: {
    ...
    var ready = function () {
      try {
        ...
        if ($$('input[name="simple_fpc"]').length > 0) {
          this.payment = $$('input[name="simple_fpc"]')[0].getAttribute(
            "data-payment_type"
          );
        }
        ...
        var appendBadgeWrapperIntoForm = function () {
          ...
          var badgeWrapper = selectParentPosition.querySelector(
            ".badge-wrapper"
          );
      ...
      var countText = function (firstRun) {
        ...
        var limitByType = function (type) {
          ...
          if (
            ...
            var msg =
              JotForm.texts[lettersWords + minMax + "LimitError"] + " " + lim;
  ...
  prePopulations: function () {
    $H(document.get).each(function (pair) {
      ...
      $$(".form-checkbox%s, .form-radio%s".replace(/\%s/gim, n)).each(function (
        input
      ) {
          ...
          for (var value; (value = values.shift()); ) {
            ...
            if (questions.length > 0) {
              ...
              questions[1].forEach(function (question_name) {
                ...
                var multilineFieldResult = multilineFieldRegex.exec(
                  question_name
                );
        ...
        .each(function (el) {
          if (["radio", "checkbox"].indexOf(el.type) > -1) {
            ...
            if (el.parentNode.dataset.type === "signaturebox") {
              // clear image as well
              var signatureImage = el.parentNode.querySelector(
                ".FITB-sign-image"
              );
    ...
    if (condition.type == "field") {
      ...
      condition.action.each(function (action) {
        ...
        if (action.visibility.toLowerCase() == "require") {
          var additionalRequireType = JotForm.getAdditionalRequiredValidationArray(
            action,
            false
          );
        ...
        if (enable) {
          ...
          if (!JotForm.isEditMode()) {
            ...
            if (isActiveSlot) {
              ...
              var richTextArea = input.parentElement.querySelector(
                ".nicEdit-main"
              );
              if (richTextArea) {
    ...
    switch (resultFieldType) {
      ...
      case "select":
        try {
          ...
          if (result.indexOf("|") > 0) {
            $("input_" + result.split("|").join("_field_")).setValue(
              out.strip()
            );
          } else {
  ...
  countTotal: function (prices) {
    ...
    if (JotForm.discounts.total) {
      if (subTotal >= minimum) {
        var reduce =
          type === "fixed" ? rate : (rate / 100) * parseFloat(subTotal);
    ...
    totalWithoutDiscount =
      flatShipping > 0
        ? totalWithoutDiscount + flatShipping
        : totalWithoutDiscount;
    ...
    if (
      ...
      var reduce =
        type === "fixed" ? rate : (rate / 100) * parseFloat(shippingTotal);
    ...
    var discount = parsePriceWithoutComma(
      Math.abs(itemTotal + shipping + taxTotal - total)
    );
    ...
    $H(prices).each(function (pair) {
      ...
      if (pair.value.tax) {
        ...
        if ($$('select[id*="input_' + surcharge.field + '"]').length > 0) {
          $$('select[id*="input_' + surcharge.field + '"]')[0].stopObserving(
            "change"
          ); // prevent stacking of listeners
          $$('select[id*="input_' + surcharge.field + '"]')[0].observe(
            "change",
            function () {
        ...
        if ($$('input[id="input_' + surcharge.field + '"]').length > 0) {
          $$('input[id="input_' + surcharge.field + '"]')[0].stopObserving(
            "keyup"
          ); // prevent stacking of listeners
          $$('input[id="input_' + surcharge.field + '"]')[0].observe(
            "keyup",
            function () {
              setTimeout(JotForm.countTotal(), 500);
            }
  ...
  setStripeSettings: function (
    ...
    if (pubkey) {
      ...
      var scriptVersion =
        scaTemplate && scaTemplate.querySelector(".cc_numberMount")
          ? "v3"
          : "v1";
      this.loadScript(
  ...
  highLightLines: function () {
    ...
    $$(".form-line").each(function (l, i) {
      l.select(
        'input, select, textarea, div, table div, button, div div span[tabIndex]:not([tabIndex="-1"])'
      ).each(function (i) {
    ...
    $$(".form-spinner-input").each(function (item) {
      ...
      item.validateSpinnerInputs = function () {
        ...
        if ((min_val || min_val == 0) && userInput < min_val) {
          return JotForm.errored(
            item,
            JotForm.texts.inputCarretErrorA + " " + min_val
          );
        } else if ((max_val || max_val == 0) && userInput > max_val) {
          return JotForm.errored(
            item,
            JotForm.texts.inputCarretErrorB + " " + max_val
          );
        } else {
          //remove error display
          return JotForm.corrected(item);
          ...
          if (timeWrapper && timeWrapper.className.indexOf("hasAMPM") > 0) {
            inputs[i].addEventListener("blur", function (e) {
              if (!userClickDetected && !userTouchDetected) {
                ...
                var timeDropdown = document.getElementById(
  ...
  initHeaderSection: function (pageArr) {
    ...
    for (var pI = 0; pI < pageArr.length; pI++) {
      if (pI > 0) {
        ...
        clonedHeader.innerHTML = selectedHeaderWrapper.querySelector(
          ".form-header-group"
  ...
  handleSignatureEvents: function () {
    ...
    if (signatureElems && signatureElems.length > 0) {
      Array.from(signatureElems).forEach(function (signatureElem) {
        ...
        var clearButton = signatureElem.querySelector(
  ...
  handleFITBInputs: function () {
    ...
    if (fitbSignatures && fitbSignatures.length > 0) {
      Array.from(fitbSignatures).forEach(function (inputContainer) {
        ...
        var signatureTrigger = inputContainer.querySelector(
          ".FITB-sign-button"
        );
        if (
          signatureImage &&
  ...
  errored: function (input, message) {
    ...
    insertEl.insert(
      new Element("div", {
        ...
        .insert(
          '<img src="' + preLink + 'images/exclamation-octagon.png"> ' + message
        )
        .insert(
  ...
  updateErrorNavigation: function (render) {
    ...
    JotForm.renderErrorNavigation =
      JotForm.renderErrorNavigation && typeof render === "undefined" && !nav
        ? true
        : render;
  ...
  toggleDisableSubmitMessage: function () {
    ...
    if (
      ...
      var jfCardElement = document.getElementsByClassName("jfCard")[
        submitButtonCardIndex
      ];
      ...
      disableSubmitErrorMessageElement.style.display = JotForm.disableSubmitButton
        ? "block"
        : "none";
        ...
        if (JotForm.isEncrypted) {
          ...
          $A(JotForm.conditions).each(function (condition) {
            if (!condition.disabled && condition.type === "url") {
              redirectConditions[condition.id] = JotForm.checkCondition(
                condition
              );
            }
          });
    ...
    input.validateInput = function (deep, dontShowMessage) {
      ...
      if (vals.include("minSelection") || vals.include("minselection")) {
        ...
        if (numberChecked > 0 && numberChecked < minSelection) {
          return JotForm.errored(
            input,
            JotForm.texts.minSelectionsError.replace(/\s*$/, "") +
              " " +
              minSelection +
              ".",
            dontShowMessage
      ...
      if (vals.include("maxselection")) {
        ...
        if (numberChecked > maxSelection) {
          return JotForm.errored(
            input,
            JotForm.texts.maxSelectionsError.replace(/\s*$/, "") +
              " " +
              maxSelection +
              ".",
            dontShowMessage
      ...
      if (vals.include("required")) {
        ...
        if (input.tagName == "INPUT" && input.readAttribute("type") == "file") {
          ...
          if (document.getElementsByName(otherInputName)[0]) {
            ...
            checkboxArray[checkboxArray.length] = document.getElementsByName(
              otherInputName
            )[0];
